...
(function(root) {
  ...
  const definitions=[
    ...
    ['nightCount','Nocturia episodes','events','Night','Mock diary','Voids marked as awakenings during the sleep opportunity. First morning void is excluded.','Count nocturnal events'],
    ['sleepHours','Sleep opportunity','h','Night','Mock schedule','Intended sleep interval, not measured sleep time.','(final awakening − sleep start) / 60'],
    ['firstSleep','First uninterrupted interval','h','Night','Mock diary','Time from sleep start to the first nocturnal void, or final awakening when there are no nocturnal voids. Does not rule out other awakenings.','First nocturnal void / wake − sleep start'],
    ['peakFullness','Peak bladder fullness','%','Filling','Sensor estimate','Peak stored volume relative to the configured capacity. Capacity is a synthetic model input.','100 × MAX(volume) / capacity'],
    ...
    ['urgeFullness','Fullness during urge proxy','%','Filling','Experimental','Mean fullness while HRV <30 ms and fullness >70%. This is a synthetic threshold rule, not a validated urgency detector.','100 × MEAN(fullness where urge flag = 1)'],
    ...
    ['diaryUrgency','Reported urgency','events','Filling','Mock diary','Explicit mock diary reports, independent of the HRV rule.','Count diary urgency entries'],
    ['voidCount','Total voids','events','Voiding','Mock diary','All known events in (recording start, recording end].','Count included events'],
    ['dayCount','Waking void frequency','events','Voiding','Mock diary','All non-nocturnal voids, including pre-sleep and first morning. Uses the sleep schedule, not fixed clock hours.','Count events with role ≠ nocturnal'],
    ...
    ['duration','Mean void duration','min','Voiding','Mock timing','Explicit synthetic stopwatch duration for each void. Cannot be resolved from one-minute bladder snapshots.','MEAN(duration seconds) / 60'],
    ['flowSurrogate','Mean flow surrogate','mL/s','Voiding','Experimental','Mean of volume / duration for each event, using mock timing. This is average flow, not peak flow.','MEAN(event volume / event seconds)'],
    ...
    ['q2','IPSS · Q2 frequency','/5','Symptoms','Mock questionnaire','Explicit past-month questionnaire response. A one-day void count cannot supply this score.','Mock questionnaire Q2 response'],
    ['q7','IPSS · Q7 nocturia','/5','Symptoms','Mock questionnaire','Explicit past-month nocturia response, not automatically derived from this single night.','Mock questionnaire Q7 response'],
    ['ipssPartial','IPSS · Q2 + Q7','/10','Symptoms','Mock questionnaire','Two domains only. This is not a total IPSS; five symptom questions are absent.','Q2 + Q7'],
    ...
    ['uui','Urge-associated leaks','events','Symptoms','Mock diary','Explicit leakage entries accompanied by urgency. No leakage volume is inferred from impedance.','Count diary entries with urgency AND leak'],
    ...
    ['fatTrend','8-week impedance reduction','%','Context','Experimental','Baseline-relative change in independent synthetic weekly impedance inputs. A negative reduction means impedance increased. This does not establish fat loss or treatment response.','100 × (week 0 − week 8) / week 0'],
    ['heartRate','Mean heart rate','bpm','Context','Mock context','Average of synthetic minute-level heart-rate values.','MEAN(heart_rate_bpm)'],
    ['hrv','Minimum HRV · RMSSD','ms','Context','Mock context','Minimum synthetic minute-level RMSSD. HRV alone does not identify urgency.','MIN(hrv_rmssd_ms)']
